...
if __name__ == "__main__":
    ...
    def actualizar_vuelo(self, vuelo):
        ...
        if vuelo['progreso'] >= 1.0:
            ...
            # Enviar mensaje de llegada (solo actualiza hora de llegada en BD)
            self.enviar_mensaje({
                'tipo': 'vuelo_completado',
                'vuelo': vuelo
            })
            ...
            # Agregar punto a la trayectoria (siempre mantener toda la trayectoria)
            vuelo['trayectoria'].append([lat, lon])
            
            # Limitar tamaño de trayectoria para optimización (mantener últimos 10000 puntos)
            # pero siempre enviar la trayectoria completa en cada actualización
            if len(vuelo['trayectoria']) > 10000:
                # Mantener puntos más recientes pero conservar algunos antiguos para visualización
                puntos_antiguos = vuelo['trayectoria'][::100]  # Cada 100 puntos
                puntos_recientes = vuelo['trayectoria'][-9000:]  # Últimos 9000
                vuelo['trayectoria'] = puntos_antiguos + puntos_recientes
    ...
    def loop_simulacion(self):
        ...
        print(f"   Aeropuertos disponibles: {len(self.aeropuertos)} en todo el mundo")
        print(f"   Rango permitido: 50 - 50,000 vuelos")
        print(f"   Factor de tiempo: x{self.FACTOR_TIEMPO} | Tick: {self.DT}s")
        ...
        while self.running:
            ...
            with self.lock:
                if len(self.vuelos_activos) < self.max_vuelos:
                    vuelos_a_generar = min(5, self.max_vuelos - len(self.vuelos_activos))
